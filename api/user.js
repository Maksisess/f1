...
module.exports = async (req, res) => {
  try {
    ...
    if (action === "pvpGetResumeTarget") {
      ...
      //  - kind:"active" — у игрока есть комната со status=active В БД;
      //  - kind:null     — возобновлять нечего.
      // ВАЖНО: здесь НЕ делаем догон/финализацию (в отличие от pvpGetMyActiveRoom). Иначе проверка
      // сама доигрывала бы пропущенные ходы до match_over и переводила комнату в finished прямо тут
      // (особенно в бот-матчах, где больше никто не поллит) — и оболочка переставала пускать игрока
      // внутрь, показывая меню. Решение о «доиграть/показать результат/продолжить» принимает уже сам
      // игровой клиент через свой polling (pvpGetRoomState) ПОСЛЕ захода: он либо продолжит матч, либо
      // покажет экран результата. Бонус: доигранный в самом приложении матч уже finished в БД, поэтому
      // сюда не попадёт — нет «отскока» обратно на экран результата после обычной игры.
      const verified = verifyTelegramInitData(req.body?.initData || "", BOT_TOKEN);
      if (!verified.ok) return res.status(401).json({ ok: false, error: verified.error });
      const tgId = String(verified.user.id);

      const rows = await sb(
        `pvp_rooms?status=eq.active&or=(player1_tg_user_id.eq.${encodeURIComponent(tgId)},player2_tg_user_id.eq.${encodeURIComponent(tgId)})&select=id,game_key,status,stake_ton,player1_tg_user_id,player2_tg_user_id,player1_name,player2_name,updated_at&order=updated_at.desc&limit=1`
      );
